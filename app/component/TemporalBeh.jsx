  ...
  return (
    <div className='min-h-screen bg-white rounded-lg p-5'>
        ...
        <h1 className='text-3xl font-semibold flex items-center justify-center text-green-500 mt-10'>Composition of Disaster</h1>
              <div className='bg-cover flex flex-wrap flex-col grid grid-cols-1 items-center justify-center border border-green-400 mt-10 w-full border'>
                    {data.map((item,id) =>{
                      return (
                      <div className='p-5 w-full flex items-center justify-center'>
                      <Image key={item.id} src={item.image} alt={item.alt} width={1000} height={1000} className='w-[1000px]'/>
                      </div>
                      )
                    })}
              </div>
